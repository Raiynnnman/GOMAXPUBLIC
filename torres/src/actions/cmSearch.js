...
export const RECEIVED_CM_DATA_SUCCESS = 'RECEIVED_CM_DATA_SUCCESS';
export const RECEIVING_CM_DATA = 'RECEIVING_CM_DATA';

export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_CM_DATA_SUCCESS,
        payload
    }
}

export function cmSearch(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({
        ...
        }).post('/office/procedures/search',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_CM_DATA_SUCCESS,
                payload: e.data.data
