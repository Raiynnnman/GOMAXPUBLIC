...
export const RECEIVED_MYDSS_DATA_SUCCESS = 'RECEIVED_MYDSS_DATA_SUCCESS';
export const RECEIVING_MYDSS_DATA = 'RECEIVING_MYDSS_DATA';
...
export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_MYDSS_DATA_SUCCESS,
        payload
    }
}

export function saveMyDaySchedule(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({
        ...
        }).post('/myday/schedule/update',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_MYDSS_DATA_SUCCESS,
                payload: e.data.data
