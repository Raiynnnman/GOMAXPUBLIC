...
export const RECEIVED_UTRAN_DATA_SUCCESS = 'RECEIVED_UTRAN_DATA_SUCCESS';
export const RECEIVING_UTRAN_DATA = 'RECEIVING_UTRAN_DATA';
...
export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_UTRAN_DATA_SUCCESS,
        payload
    }
}
...
export function receivingData(params) {
  return async (dispatch) => {
    ...
    const response = await axios.create({
        ...
        }).post('/office/transfers/list',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_UTRAN_DATA_SUCCESS,
                payload: e.data.data
