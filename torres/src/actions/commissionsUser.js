...
export const RECEIVED_COMMU_DATA_SUCCESS = 'RECEIVED_COMMU_DATA_SUCCESS';
export const RECEIVING_COMMU_DATA = 'RECEIVING_COMMU_DATA';

export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_COMMU_DATA_SUCCESS,
        payload
    }
}

export function getCommissionUserAdmin(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({
        ...
        }).post('/admin/commission/user/list',params)
      .then((e) => { 
          var g = e.data.data;
          if (g.content) { 
