...
export const RECEIVED_CHUD_DATA_SUCCESS = 'RECEIVED_CHUD_DATA_SUCCESS';
export const RECEIVING_CHUD_DATA = 'RECEIVING_CHUD_DATA';
...
export function receiveDataSuccess(payload) {
    return {
        type: RECEIVED_CHUD_DATA_SUCCESS,
        payload
    }
}

export function chatUploadDoc(params,callback,args) { 
...
export function receivingData(params,callback,args) {
  return async (dispatch) => {
    ...
    const response = await axios.create({
        ...
        }).post('/chat/document/update',params)
      .then((e) => { 
          dispatch({
                type: RECEIVED_CHUD_DATA_SUCCESS,
                payload: e.data.data
