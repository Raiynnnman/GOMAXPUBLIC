...
from util.Permissions import check_consultant
...
class ConsultantBillingDownloadDoc(ConsulantBase):
    def __init__(self):
        super().__init__()
    ...
    def execute(self, *args, **kwargs):
        ...
        o = db.query("""
            select 
                mimetype,blob_path 
            from
                consultant_invoice_upload_documents
            where
                consultant_user_id = %s and
                id = %s
...
class ConsultantBilling(ConsulantBase):
    def __init__(self):
        super().__init__()
...
class ConsultantDashboard(ConsulantBase):
    def __init__(self):
        super().__init__()
    ...
    def getRevenueThisMonth(self,user_id):
        ...
        o = db.query("""
            ...
            from 
                (select round(sum(price),2) as num1 from consultant_fee_items a where consultant_user_id = %s and
                    a.created > date_add(date_add(LAST_DAY(now()),interval 1 DAY),interval -1 MONTH)) as t1,
                (select count(id) as num2 from consultant_fee_items b where consultant_user_id = %s and
                    b.created > date_add(date_add(LAST_DAY(now()),interval 1 DAY),interval -1 MONTH)) as t2,
                (select count(css.id) as num3 from consultant_schedule_scheduled css,consultant_schedule cs where 
                    css.consultant_schedule_id = cs.id and cs.user_id=%s and
                    cs.tstamp > date_add(date_add(LAST_DAY(now()),interval 1 DAY),interval -1 MONTH)) as t3,
                (select count(cp.id) as num4 from consultant_payouts cp,consultant_transfers ct where cp.consultant_transfers_id = ct.id and 
                    consultant_user_id=%s and ct.created > date_add(date_add(LAST_DAY(now()),interval 1 DAY),interval -1 MONTH)) as t4
...
class ConsultantConfig(ConsulantBase):
    ...
    def getInvoices(self,id,db):
        inv = db.query("""
            ...
            from
                ...
                invoice_status ist
            where
                css.consultant_schedule_id = %s and
    ...
    def execute(self, *args, **kwargs):
        ...
        s_conf = db.query("""
            ...
            from
                consultant_schedule_config csc,
                users u
            where 
        ...
        upc = db.query("""
            ...
            where
                cs.user_id = %s and 
                cs.id in (select consultant_schedule_id from 
                    consultant_schedule_scheduled) and
        ...
        for x in o:
            ...
            for j in sched:
                ...
                c1 = db.query("""
                    ...
                    physician_appt_comments pa,consultant_schedule_scheduled css
                    where 
...
class UpdateSchedule(ConsulantBase):
    ...
    def execute(self, *args, **kwargs):
        ...
        if 'id' in params:
            ...
            db.update("""
                update consultant_schedule_config set updated=now(),
            ...
            db.update("""
                insert into consultant_schedule_config(
        ...
        db.update("""
            delete from consultant_schedule cs where consultant_schedule_config_id = %s and
                cs.id not in (select consultant_schedule_id from consultant_schedule_scheduled)
            """,(insid,)
        )
        db.commit()
        return {'success': True}
