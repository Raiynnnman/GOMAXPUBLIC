# coding=utf-8

import os
import sys
import json
import unittest
import base64
import pickle
import copy
import traceback
from util import encryption, S3Processing
from processing.run import app
from flask import request, jsonify
from common.DataException import DataException
from common.InvalidParameterException import InvalidParameterException
from common import settings
from processing import JobStates
from processing.JavascriptEngine import JavascriptEngine
from processing.SavedFileData import SavedFileData
from processing.ThrottleControl import ThrottleControl
from processing.ProcessingBase import ProcessingBase
from py4j.protocol import Py4JJavaError

config = settings.config()
config.read("settings.cfg")


@app.task(bind=True)
def submit_task_wrapper(self, *args):
    if args is None or len(args) < 1:
        raise EngineException("MISSING_ARGUMENT_TO_QUEUE")
    a = copy.deepcopy(args[0])
    if a is None or len(a) < 1:
        raise EngineException("MISSING_ARGUMENT_TO_QUEUED_INSTANCE")

    myc = args[0][0]
    if myc.startswith("{"):
        myc = json.loads(myc)
    dp = pickle.loads(
        base64.b64decode(myc.encode('utf-8'))
    )
    a.pop(0)
    try:
        if isinstance(a, str):
            a = json.loads(a)
        dp.run(a)
    except DataException as de:
        raise de
    except InvalidParameterException as ipe:
        raise ipe
    except Py4JJavaError as p4j:
        raise p4j
    except Exception as e:
        exc_type, exc_value, exc_traceback = sys.exc_info()
        print(str(e)) # EXCEPTION
        traceback.print_tb(exc_traceback, limit=100, file=sys.stdout)
        raise self.retry(exc=e, countdown=60, args=args)

class SubmitDataRequest(ProcessingBase):

    def __init__(self):
        super().__init__()

    def isDeferred(self):
        return True

    __jobid = 0

    def defer(self, data):
        dp = base64.b64encode(pickle.dumps(self)).decode('utf-8')
        nargs = (dp, data)
        try:
            qitem = submit_task_wrapper.delay(nargs)
            qitem.forget()
        except NotImplementedError as NIE:
            # The backend doesnt have forget, pass on it
            pass
        except Exception as e:
            raise e

    def handleFilters(self, table, filters, mydata):
        mysubmitdata = []
        if 'filters' in filters:
            for outer in filters['filters']:
                if outer['isActive'] != 1:
                    # print("filter inactive")
                    continue
                if outer['database'] != table:
                    # print("wrong database")
                    continue
                if 'filters' in outer:
                    js = JavascriptEngine()
                    for filts in outer['filters']:
                        j = js.process(self.__jobid, filts['name'], filts['script'],mydata)
                        mysubmitdata.append(j)
        else:
            mysubmitdata = [mydata]
        return mysubmitdata
		
    def getLimits(self, args):
        limits = {}
        limits['limit'] = args[0].get('limit', 10)
        limits['page'] = args[0].get('page', 0)
        limits['offset'] = limits['limit'] * limits['page']
        return limits

    def execute(self, *args, **kwargs):
        raise Exception("DERIVED_CLASS_MUST_IMPLEMENT")

    def process(self, *args, **kwargs):
        jobstate = JobStates.JobStates()
        jobstate.jobQueued()
        if self.isDeferred():
            throttle = ThrottleControl()
            isThrottled = throttle.process(self.__class__.__name__)
            if isThrottled:
                return {"success":False,"throttled":True}
        if not self.isDeferred():
            jobstate.setIsActive(0)
        proc = []
        if len(args) > 0:
            proc = args[0]
        path = "%s/%s.json" % (
            "requests", 
            encryption.getSHA256(encryption.getRandomChars(r=64))
        )
        S3Processing.uploadS3ItemToBucket(
            config.getKey("request_bucket_access_key"),
            config.getKey("request_bucket_access_secret"),
            config.getKey("request_bucket"),
            path,
            json.dumps(proc)
        )
        if 'DataRequest' in self.__class__.__name__:
            sfd = SavedFileData()
            sfd.process(jobstate.getJobID(), self.__class__.__name__, path)
        d = { "s3data": path, "jobid":jobstate.getJobID() }
        ret = {}
        if self.isDeferred() and config.getKey("use_defer"):
            self.defer(d)
            ret = {"jobid": jobstate.getJobID()}
        else:
            ret = self.run(d)
        return ret

    def run(self, *args, **kwargs):
        task = args[0]
        if isinstance(task, str):
            task = json.loads(task)
        if isinstance(task, list):
            task = task[0]
        jobstate = JobStates.JobStates()
        jobstate.setJobID(task['jobid'])
        self.__jobid = task['jobid']
        data = task
        s3path = None
        jobstate.jobStarted()
        if 's3data' in data:
            s3path = data['s3data']
            data = S3Processing.downloadS3ItemFromBucket(
                    config.getKey("request_bucket_access_key"),
                    config.getKey("request_bucket_access_secret"),
                    config.getKey("request_bucket"),
                    s3path
            )
        jobstate.jobRunning()
        try:
            if isinstance(data, str):
                data = json.loads(data)
            ret = self.execute(data, task['jobid'])
        except Exception as e:
            jobstate.jobError()
            raise e
        jobstate.jobComplete()
        # Dont delete the file at the end
        if 'DataRequest' not in self.__class__.__name__ and s3path is not None:
            try:
                S3Processing.deleteS3ItemFromBucket(
                    config.getKey("request_bucket_access_key"),
                    config.getKey("request_bucket_access_secret"),
                    config.getKey("request_bucket"),
                    s3path
                )
            except:
                print("ERROR: Failed to delete item from bucket")
        return ret
