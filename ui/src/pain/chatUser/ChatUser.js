...
class ChatUser extends Component {
    ...
    componentDidMount() {
        this.props.dispatch(getChatUser())
    }
    ...
    render() {
        ...
        <>
          ...
          <div className={`chat-page-wrapper ${s.chatPage} ${mobileState === MobileChatStates.LIST ? 'list-state' : ''} ${mobileState === MobileChatStates.CHAT ? 'chat-state' : ''} ${mobileState === MobileChatStates.INFO ? 'info-state' : ''}`}>
                ...
                <>
                    {(window.innerWidth > 1024) && (<ChatList onNewChat={this.onNewChat} data={this.props.chatUser.data}/>)}
                    <ChatDialog data={this.props.chatUser.data}/>
                    {(window.innerWidth > 1024) && ( <ChatInfo data={this.props.chatUser.data}/>)}
                </>
                )}
          </div>
        </>
