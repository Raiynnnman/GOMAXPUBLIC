...
class JobStates(ProcessingBase):
    ...
    def initializeJobID(self):
        ...
        mydb = DBManager().getConnection()
        curs = mydb.cursor()
        query = "insert into jobs (job_id, curr_status, is_active) values (%s, %s, %s)"
        curs.execute(query, (jobname, 0, self.getIsActive()));
    ...
    def jobComplete(self):
        ...
        if random.randint(0,100) > 75:
            ...
            mydb.commit()
        if random.randint(0,100) > 75:
            query = "delete from results where job_id not in (select id from jobs);"
            curs.execute(query)
            mydb.commit()
        if random.randint(0,100) > 75:
            query = "delete from result_rows where resultid not in (select id from results);"
            curs.execute(query)
            mydb.commit()
        if random.randint(0,100) > 75:
            query = "delete from files where job_id not in (select id from jobs);"
            curs.execute(query)
            mydb.commit()
        mydb.close()
